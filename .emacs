...
; UI: Themes
(cond (t (progn (load-theme 'base16-tomorrow t)
                (set-face-background 'fringe "white")))
      (nil (load-theme 'gruvbox t))
      (nil ((setq x-underline-at-descent-line t)
...
(defun god-mode-update-cursor ()
  ...
  (let ((limited-colors-p (> 257 (length (defined-colors)))))
    (cond (god-local-mode (progn
                            (set-face-background 'mode-line "Grey97")
                            (set-face-background 'mode-line-inactive "Grey98")))
          (t (progn
               (set-face-background 'mode-line "Grey94")
               (set-face-background 'mode-line-inactive "Grey93"))))))

(defun god-mode-update-cursor-old ()
  (setq cursor-type (if (or god-local-mode buffer-read-only)
                        'box
                      'bar)))
